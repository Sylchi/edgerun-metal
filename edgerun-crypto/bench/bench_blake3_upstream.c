...
static void bench_blake3_hash(const uint8_t* bytes, size_t len, uint8_t digest[BLAKE3_OUT_LEN]) {
...
static uint8_t bench_run_case_once(const BenchCase* c, const uint8_t* bytes, uint64_t* out_elapsed_ns) {
  ...
  for (i = 0u; i < c->iterations; ++i) {
    bench_blake3_hash(bytes, c->bytes_len, digest);
    g_bench_sink ^= digest[i & (BLAKE3_OUT_LEN - 1u)];
  }
  end = bench_now_ns();
  if (end <= start) {
    return 0u;
...
int main(void) {
  static const BenchCase cases[] = {
    {"small", 64u, 500000u},
    {"chunk", BLAKE3_CHUNK_LEN, 100000u},
    {"64k", 65536u, 5000u},
    {"1m", 1048576u, 500u},
    {"8m", 8388608u, 64u},
    {"64m", 67108864u, 8u}
...
#if defined(BLAKE3_USE_TBB)
  printf("BLAKE3 benchmark (upstream %s, tbb)\n", blake3_version());
#else
  printf("BLAKE3 benchmark (upstream %s, amd64-asm)\n", blake3_version());
#endif
  for (i = 0u; i < sizeof(cases) / sizeof(cases[0]); ++i) {
