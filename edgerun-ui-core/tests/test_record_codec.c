...
static const size_t ER_TEST_RECORD_BUFFER_CAPACITY = 256u;
static const size_t ER_TEST_RECORD_STRING_SLOT_LEN = 4u;
static const size_t ER_TEST_RECORD_VEC_COUNT = 2u;
static const size_t ER_TEST_RECORD_SINGLE_CHAR_LEN = 1u;
static const uint64_t ER_TEST_RECORD_WIDE_VALUE = 42u;
static const size_t ER_TEST_RECORD_BAD_MAGIC_LEN = 3u;
static const size_t ER_TEST_RECORD_SMALL_BUFFER_CAPACITY = 8u;
static const size_t ER_TEST_RECORD_LONG_STRING_LEN = 7u;
static const uint8_t ER_TEST_RECORD_ARRAY_FILL = 7u;
static const uint8_t ER_TEST_RECORD_INVALID_UTF8_BYTE = 0x80u;
...
static void test_record_codec_round_trips_values(void) {
  uint8_t bytes[ER_TEST_RECORD_BUFFER_CAPACITY];
  ...
  for (size_t i = 0u; i < ER_UI_RECORD_ARRAY_32_LEN; ++i) seven[i] = ER_TEST_RECORD_ARRAY_FILL;

  expect_record_status(er_ui_record_write_string(&writer, "slot", ER_TEST_RECORD_STRING_SLOT_LEN), ER_UI_RECORD_OK, "record: string writes");
  ...
  expect_record_status(er_ui_record_write_vec_len(&writer, ER_TEST_RECORD_VEC_COUNT), ER_UI_RECORD_OK, "record: bytes vec count writes");
  ...
  expect_record_status(er_ui_record_write_vec_len(&writer, ER_TEST_RECORD_VEC_COUNT), ER_UI_RECORD_OK, "record: string vec count writes");
  expect_record_status(er_ui_record_write_string(&writer, "a", ER_TEST_RECORD_SINGLE_CHAR_LEN), ER_UI_RECORD_OK, "record: first string vec element writes");
  expect_record_status(er_ui_record_write_string(&writer, "b", ER_TEST_RECORD_SINGLE_CHAR_LEN), ER_UI_RECORD_OK, "record: second string vec element writes");
  ...
  expect_record_status(er_ui_record_write_u64(&writer, ER_TEST_RECORD_WIDE_VALUE), ER_UI_RECORD_OK, "record: u64 writes");
  ...
  expect_string_slice(str, str_len, "slot", ER_TEST_RECORD_STRING_SLOT_LEN, "record: string value round trips");
  ...
  expect_size(count, ER_TEST_RECORD_VEC_COUNT, "record: bytes vec count");
  ...
  expect_size(count, ER_TEST_RECORD_VEC_COUNT, "record: string vec count");
  expect_record_status(er_ui_record_read_string(&reader, &str, &str_len), ER_UI_RECORD_OK, "record: first string vec reads");
  expect_string_slice(str, str_len, "a", ER_TEST_RECORD_SINGLE_CHAR_LEN, "record: first string vec value");
  expect_record_status(er_ui_record_read_string(&reader, &str, &str_len), ER_UI_RECORD_OK, "record: second string vec reads");
  expect_string_slice(str, str_len, "b", ER_TEST_RECORD_SINGLE_CHAR_LEN, "record: second string vec value");
  ...
  expect_true(wide == ER_TEST_RECORD_WIDE_VALUE, "record: u64 value round trips");
...
static void test_record_codec_rejects_bad_records(void) {
  er_ui_record_reader_t reader = {0};
  expect_record_status(er_ui_record_reader_init(&reader, (const uint8_t*)"BAD", ER_TEST_RECORD_BAD_MAGIC_LEN, ER_TEST_RECORD_MAGIC, sizeof(ER_TEST_RECORD_MAGIC)),
  ...
  uint8_t invalid_utf8[] = {'T', 'E', 'S', 'T', '1', ER_TEST_RECORD_SINGLE_CHAR_LEN, 0u, 0u, 0u, ER_TEST_RECORD_INVALID_UTF8_BYTE};
...
static void test_record_codec_limits_and_capacity(void) {
  ...
  uint8_t small[ER_TEST_RECORD_SMALL_BUFFER_CAPACITY];
  er_ui_record_writer_t writer = {0};
  ...
  expect_record_status(er_ui_record_write_string(&writer, "toolong", ER_TEST_RECORD_LONG_STRING_LEN), ER_UI_RECORD_ERR_NO_SPACE,
                       "record: writer reports capacity exhaustion");
