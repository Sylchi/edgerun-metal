...
static void test_text_measure_empty_and_rejects_invalid_inputs(void) {
  ...
  expect_status(er_ui_varfont_measure_text(face, text, 4u, NULL), ER_UI_ERR_INVALID_ARGUMENT,
                "text measure: missing metrics output is rejected");
  expect_status(er_ui_varfont_measure_text(NULL, text, 4u, &metrics), ER_UI_ERR_INVALID_ARGUMENT,
                "text measure: missing face is rejected");
  expect_status(er_ui_varfont_measure_text(face, NULL, 4u, &metrics), ER_UI_ERR_INVALID_ARGUMENT,
...
static void test_text_vertex_and_ascii_helpers_validate_boundaries(void) {
  ...
  expect_status(er_ui_scene_push_varfont_vertices(&scene, partial, 2u, ER_TEST_TEXT_COLOR), ER_UI_ERR_INVALID_ARGUMENT,
  ...
  if (face) {
    ...
    expect_status(er_ui_scene_push_ascii_text(&scene, face, "Edge", 4u, 4.0f, 48.0f, ER_TEST_TEXT_COLOR), ER_UI_OK,
                  "text helpers: ascii text fits exact budget");
    expect_true(scene.text_quad_count > 0u, "text helpers: ascii text emits quads");
    expect_status(er_ui_scene_push_ascii_text(&scene, face, "Edge", 3u, 4.0f, 72.0f, ER_TEST_TEXT_COLOR),
                  ER_UI_ERR_INVALID_ARGUMENT, "text helpers: ascii text rejects undersized budget");
    expect_status(er_ui_scene_push_ascii_text(&scene, face, NULL, 4u, 4.0f, 96.0f, ER_TEST_TEXT_COLOR),
