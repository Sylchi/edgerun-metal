...
Any events _not_ listed in the SETEVENTS line are turned off; thus, sending SETEVENTS with an empty body turns off all event reporting.
...
This feature is designed to be used to help Tor-ify applications that need to use SOCKS4 or hostname-less SOCKS5. There are three approaches to doing this:
...
[code]
        ...
        "entry-guards"
          ...
          0.2.2.1-alpha and later by default, each line is of the form:
             LongName SP Status [SP DateSpTime] CRLF

         In Tor versions 0.1.2.2-alpha through 0.2.2.1-alpha with feature
         VERBOSE_NAMES turned off and before version 0.1.2.2-alpha, each line
         is of the form:
             ServerID2 SP Status [SP DateSpTime] CRLF
        ...
        "bw-event-cache"
         ...
         "consensus/valid-until"
          Each of these produces a DateSpTime describing part of the lifetime of
        ...
        "downloads/"
          ...
          a "serialized download status" as follows:
           ...
           SerializedDownloadStatus =
             ...
             "next-attempt-at" SP DateSpTime CRLF
        ...
        "current-time/local"
        "current-time/utc"
          The current system or UTC time, as returned by the system, in DateTime
...
HSAddress can be version 2 or version 3 addresses. DescIDs can only be version 2 IDs. Version 2 addresses consist of 16 _Base32Character and version 3 addresses consist of 56_ Base32Character.
...
[code]
        ...
        PowQRate = The sustained rate of rendezvous requests to dispatch per
                   second from the priority queue. Default value 250.

        PowQBurst = The maximum burst size for rendezvous requests handled from
                    the priority queue at once. Default is 2500.
...
On success, Tor replies `250 OK` then Tor MUST eventually follow this with an HS_DESC event with the result for each upload location.
...
“DateTimeFrac” is a time format based on DateTime:
[code]
    DateTimeFrac = DateTime [ "." 1*DIGIT ]

[/code]

  * Example DateTimeFrac: “2012-01-11T12:15:33.51”
...
If clients receives extended events (selected by USEFEATUERE EXTENDED_EVENTS in Tor 0.1.2.2-alpha..Tor-0.2.1.x, and always-on in Tor 0.2.2.x and later), then each event line as specified below may be followed by additional arguments and additional lines.
...
The “REND_QUERY” field is provided only for hidden-service-related circuits, and only in versions 0.2.3.11-alpha and later. Clients MUST accept hidden service addresses in formats other than that specified above.
[Added in Tor 0.4.3.1-alpha.]

The “TIME_CREATED” field is provided only in versions 0.2.3.11-alpha and later. TIME_CREATED is the time at which the circuit was created or cannibalized.
[Added in Tor 0.4.3.1-alpha.]
...
In Tor versions 0.1.2.2-alpha through 0.2.2.1-alpha with feature VERBOSE_NAMES turned off and before version 0.1.2.2-alpha, OR Connection is as follows:
[code]
...
In Tor versions 0.1.2.2-alpha through 0.2.2.1-alpha with feature VERBOSE_NAMES turned off and before version 0.1.2.2-alpha, it is as follows:
[code]
...
[code]
         ...
         Expiry = DQUOTE DateSpTime DQUOTE / "NEVER"

         ErrorCode = "yes" / "internal" / "Unable to launch resolve request"
         UTCExpiry = DQUOTE DateSpTime DQUOTE
...
[code] "Progress" gives a number between 0 and 100 for how far through
...
[code] {Controllers may want to use this event to decide when to indicate
...
[code] {Suggested use: controllers can notify their users that Tor is
...
[code] {Controllers should warn their users when this occurs, unless they
...
[code] {Controllers may want to warn their users when this occurs: it
...
. `ADDRESS`=`IP`
. `HOSTNAME`=`NAME`
. `METHOD`=`CONFIGURED / CONFIGURED_ORPORT / DIRSERV / RESOLVED / INTERFACE / GETHOSTNAME`
...
[code] {Controllers may want to record this info and display it to the user.}
...
[code] {This event could affect the controller's idea of server status, but
...
[code] {Originally, the goal of this event was to declare "every authority
...
[code] {Moving from listed to unlisted is not necessarily cause for
...
[code] {This is a problem; if it happens often without the nameservers
...
[code] {This is an annoyance; controllers may want to tell admins that their
...
[code] {Controllers could warn the admin if the relay is running as an
...
[code] {Controllers should warn the admin, and try to cope if they can.}
...
[code] {This event could affect the controller's idea of server status.  The
...
Note that from Tor’s perspective, _reading_ a byte on a stream means that the application _wrote_ the byte. That’s why the order of “written” vs “read” is opposite for stream_bw events compared to bw events.
...
A new consensus networkstatus has arrived. We include NS-style lines for every relay in the consensus. NEWCONSENSUS is a separate event from the NS event, because the list here represents every usable relay: so any relay _not_ mentioned in this list is implicitly no longer recommended.
...
For “QUERY_NO_HSDIR” or “QUERY_RATE_LIMITED”, the HsDir will be set to “UNKNOWN” which was introduced in tor 0.3.1.0-alpha and 0.4.1.0-alpha respectively.
