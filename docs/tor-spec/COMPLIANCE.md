...
| App cell SDK authority | `02-tor-protocol.md`, `04-path-spec.md`, `05-guard-spec.md`, `13-onion-services.md` | Partial. Local identity routing sends unknown valid identities through the kernel public relay path by default. Existing host Tor role state remains kernel-owned implementation detail for the kernel relay path. | Replace role-specific app SDK surface with one grant-backed fixed-cell transport: app identity registration, cell send to destination identity, cell receive from own mailbox, and receipt emission. App-owned Tor roles must live inside the WASM binary and use only that cell transport. |
...
The app SDK must not expose one host import per Tor role. The app SDK exposes only the app's own identity and a fixed-cell transport:

- register the app's grant-backed identity mailbox,
- send one Tor fixed cell to a destination identity,
- receive one Tor fixed cell from the app's own mailbox,
- emit or attach the receipt required by the grant and object edge.

Client, guard, middle relay, exit, directory, bridge, introduction, rendezvous, onion-service, and peer behavior are app-owned WASM code. A WASM binary that wants any Tor role includes that role implementation in its own code and receives only the resource grants needed for its own identity, cells, memory, durable flush capacity, and key material. The system relays cells between identities; it does not become the app's role controller.

There must be one Tor-compatible protocol path, not parallel EdgeRun-specific protocol variants. All app roles use the same SDK path: Tor-compatible cells are carried through EdgeRun identity routing and local circuits. Tor is the privacy and compatibility envelope for legacy IP and app-to-app privacy paths; it is not a POSIX process model inside EdgeRun.
