...
static void test_clock_progression(void) {
  ...
  check_int("clock init", er_clock_init(&limits, &clock), ER_CLOCK_OK);
  check_int("stamp zero invalid", er_clock_stamp_valid(clock.now), 0);
...
static void test_clock_rejects_invalid(void) {
  ...
  limits.slots_per_epoch = TEST_LIMIT_TWO;
  limits.epochs_per_era = TEST_LIMIT_TWO;
  check_int("reject zero limit", er_clock_init(&limits, &clock),
            ER_CLOCK_ERR_BADARG);
  limits.ticks_per_slot = TEST_LIMIT_TWO;
  limits.slots_per_epoch = TEST_LIMIT_THREE;
  check_int("reject non-power limit", er_clock_init(&limits, &clock),
            ER_CLOCK_ERR_BADARG);
  limits.slots_per_epoch = TEST_LIMIT_FOUR;
  check_int("init modifier clock", er_clock_init(&limits, &clock), ER_CLOCK_OK);
