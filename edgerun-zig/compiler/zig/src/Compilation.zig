...
pub const Directories = struct {
    ...
    pub fn init(
        ...
        const zig_lib: Cache.Directory = d: {
            ...
            const resolved_self_exe_path = self_exe_path orelse
                fatal("unable to find zig self exe path", .{});
            break :d introspect.findZigLibDirFromSelfExe(arena, io, cwd, resolved_self_exe_path) catch |err| {
                fatal("unable to find zig installation directory '{s}': {t}", .{ resolved_self_exe_path, err });
            };
        };
