...
build_dir="../.build/edgerun-zig/immutable-kernel-swtpm"
...
mkdir -p "$efi_dir" "$swtpm_dir"
zig build-exe src/immutable_kernel_tpm_uefi.zig \
  -target x86_64-uefi \
  -O ReleaseSmall \
  -femit-bin="$efi_app"

rm -f "$esp_img" "$debug_log" "$swtpm_sock" "$swtpm_log"
rm -rf "$swtpm_dir/state"
...
cleanup() {
  if [ -S "$swtpm_sock" ]; then
    swtpm_ioctl --unix "$swtpm_sock" -s >/dev/null 2>&1 || true
  fi
}
...
if ! grep -q "PASS immutable-kernel-swtpm-qemu" "$debug_log"; then
  cat "$debug_log"
  exit 1
fi
