...
build_dir="../.build/edgerun-zig/immutable-kernel-runtime"
...
mkdir -p "$efi_dir"
zig build-exe src/immutable_kernel_runtime_uefi.zig \
  -target x86_64-uefi \
  -O ReleaseSmall \
  -femit-bin="$efi_app"
...
if ! grep -q "PASS immutable-kernel-runtime-qemu" "$debug_log"; then
  cat "$debug_log"
  exit 1
fi
