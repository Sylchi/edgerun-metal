...
The usual mistake is treating the device as one object. People say "my phone sent a message" the same way they say "my hand opened a door." But the phone is not one actor. It is a city of smaller systems, each with a job, each with a boundary, and each able to help or betray the user depending on who controls it.

> The trick: before blaming "the cloud," ask which office inside the phone first saw, stored, transformed, or leaked the data.
...
This is why "just make an app" is never just an app. The app depends on memory rules, file rules, network rules, display rules, identity rules, update rules, and platform policy. A chat message is not born in the cloud. It is a local event that may become a network event later.
...
The important question is not whether the city is complex. It is whether the user can see which office is allowed to do what.
