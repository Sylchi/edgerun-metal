...
pub const Toast = struct {
    ...
    pub fn render(self: Toast, scene: *ui.Scene, bounds: ui.Rect, options: RenderOptions) ui.RenderError!void {
        ...
        try scene.pushText(ui.Rect.init(text_x, toast.y + toast_title_y, @max(component_primitives.min_extent, toast.x + toast.w - text_x - toast_padding), toast_title_h), self.title, options.style.text);
        try scene.pushText(ui.Rect.init(text_x, toast.y + toast_detail_y, @max(component_primitives.min_extent, toast.x + toast.w - text_x - toast_padding), toast_detail_h), self.detail, options.style.muted);
    }
