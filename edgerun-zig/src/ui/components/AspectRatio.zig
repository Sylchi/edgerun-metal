...
pub const AspectRatio = struct {
    ...
    pub fn toObject(self: AspectRatio, ui_out: []u8, object_out: []u8, req: object.Requirements, epoch: clock.Stamp) ?[]u8 {
        return component_codec.refObject(.aspect_ratio, 0, .{ .offset = self.ratio_w, .len = self.ratio_h }, ui_out, object_out, req, epoch);
    }
...
test "aspect ratio component serializes to canonical object and deserializes" {
    ...
    const canonical = ratio.toObject(&ui_raw, &object_raw, component_test.req(), component_test.epoch()).?;
