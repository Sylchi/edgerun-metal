...
pub const Command = struct {
    ...
    pub fn toObject(self: Command, ui_out: []u8, object_out: []u8, req: object.Requirements, epoch: clock.Stamp) ?[]u8 {
        return component_codec.oneStringObject(.command, self.id, self.placeholder, ui_out, object_out, req, epoch);
    }
...
test "command component serializes to canonical object and deserializes" {
    ...
    const canonical = command.toObject(&ui_raw, &object_raw, component_test.req(), component_test.epoch()).?;
