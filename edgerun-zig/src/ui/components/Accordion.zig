...
pub const Accordion = struct {
    ...
    pub fn toObject(self: Accordion, ui_out: []u8, object_out: []u8, req: object.Requirements, epoch: clock.Stamp) ?[]u8 {
...
test "accordion component serializes to canonical object and deserializes" {
    ...
    const canonical = accordion.toObject(&ui_raw, &object_raw, component_test.req(), component_test.epoch()).?;
