...
pub const DropdownMenu = struct {
    ...
    pub fn toObject(self: DropdownMenu, ui_out: []u8, object_out: []u8, req: object.Requirements, epoch: clock.Stamp) ?[]u8 {
        return component_codec.twoStringObject(.dropdown_menu, self.id, self.first, self.second, ui_out, object_out, req, epoch);
    }
