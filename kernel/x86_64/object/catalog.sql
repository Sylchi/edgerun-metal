...
create table x86_xmm_register_encoding_fact (
  register_name text primary key,
  reg_code integer not null,
  requires_rex integer not null
);

insert into x86_xmm_register_encoding_fact(register_name, reg_code, requires_rex) values
  ('xmm0', 0, 0), ('xmm1', 1, 0), ('xmm2', 2, 0), ('xmm3', 3, 0),
  ('xmm4', 4, 0), ('xmm5', 5, 0), ('xmm6', 6, 0), ('xmm7', 7, 0),
  ('xmm8', 8, 1), ('xmm9', 9, 1), ('xmm10', 10, 1), ('xmm11', 11, 1),
  ('xmm12', 12, 1), ('xmm13', 13, 1), ('xmm14', 14, 1), ('xmm15', 15, 1);
...
with binary_operand as (
  ...
  where normalized.op_name in ('mov','add','sub','cmp','and','or','xor')
    and reg.width_bits in (8,16,32,64)
  ...
  where normalized.op_name in ('mov','add','sub','cmp','and','or','xor','lea','movzx','movsx','movsxd')
    and reg.width_bits in (8,16,32,64)
    and ((mem.size_name is null and normalized.op_name not in ('movzx','movsx','movsxd','lea'))
...
with clean_indexed_memory as (
  ...
  select mem.*,
         rhs_unique_value.immediate_value as rhs_immediate_value
  from indexed_memory_bytes mem
  join rhs_unique_value
    on rhs_unique_value.repo_file_id = mem.repo_file_id
   and rhs_unique_value.function_name is mem.function_name
   and rhs_unique_value.line_no = mem.line_no
  ...
  select repo_file_id,
         ...
         (
           ...
           || case
                ...
                when op_name = 'movsx' and size_name = 'word' then '0fbf'
                when op_name = 'movsxd' then '63'
                when op_name = 'mov' and width_bits = 8 then '8a'
                when op_name = 'mov' then '8b'
                when op_name = 'add' and width_bits = 8 then '02'
                when op_name = 'add' then '03'
                when op_name = 'sub' and width_bits = 8 then '2a'
                when op_name = 'sub' then '2b'
                when op_name = 'cmp' and width_bits = 8 then '3a'
                when op_name = 'cmp' then '3b'
                when op_name = 'and' and width_bits = 8 then '22'
                when op_name = 'and' then '23'
                when op_name = 'or' and width_bits = 8 then '0a'
                when op_name = 'or' then '0b'
...
create table repo_asm_memory_immediate_parametric_encoding_fact as
with recursive rhs_decimal as (
  select operand.repo_file_id,
         operand.function_name,
         operand.line_no,
  ...
  select match.repo_file_id,
         ...
         match.op_name,
         memory.size_name,
         memory.integer_displacement as displacement,
         base_reg.reg_code as base_reg_code,
         base_reg.requires_rex as base_requires_rex,
         rhs_unique_value.immediate_value
  from repo_asm_rule_match match
  join repo_asm_memory_addressing_fact memory
    on memory.repo_file_id = match.repo_file_id
   and memory.function_name = match.function_name
   and memory.line_no = match.line_no
   and memory.operand_index = 0
  join x86_register_encoding_fact base_reg
    on base_reg.register_name = memory.first_register_term
   and base_reg.width_bits = 64
  join rhs_unique_value
    on rhs_unique_value.repo_file_id = match.repo_file_id
   and rhs_unique_value.function_name = match.function_name
  ...
  where match.target_isa_id = 1
    ...
    and memory.size_name in ('byte','word','dword','qword')
), memory_bytes as (
  select memory_imm.*,
         case
           when displacement = 0 and (base_reg_code & 7) not in (5) then 0
           when displacement between -128 and 127 then 1
           else 2
         end as mod_bits,
         case
           when displacement = 0 and (base_reg_code & 7) not in (5) then ''
           when displacement between -128 and 127 then printf('%02x', displacement & 255)
           else printf('%02x%02x%02x%02x',
                       displacement & 255,
                       (displacement >> 8) & 255,
         ...
         case when (base_reg_code & 7) = 4 then '24' else '' end as sib_hex,
         case when (base_reg_code & 7) = 4 then 4 else (base_reg_code & 7) end as rm_field
  from memory_imm
), generated as (
  select repo_file_id,
         function_name,
         line_no,
         'param_x86_mov_' || size_name || '_base_memory_numeric_imm' as parametric_rule_name,
         (
           case when size_name = 'word' then '66' else '' end
           || case
                when size_name = 'qword' then printf('%02x', 72 + case when base_reg_code >= 8 then 1 else 0 end)
           ...
           || case when size_name = 'byte' then 'c6' else 'c7' end
           || printf('%02x', (mod_bits << 6) + rm_field)
           || sib_hex
           || displacement_hex
           || case
  ...
  where op_name = 'mov'
    and ((size_name = 'byte' and immediate_value between -128 and 255)
         ...
         or (size_name = 'dword' and immediate_value between -2147483648 and 4294967295)
         or (size_name = 'qword' and immediate_value between -2147483648 and 2147483647))
  union all
  select repo_file_id,
         function_name,
         line_no,
...
create unique index repo_asm_memory_immediate_parametric_encoding_fact_line_idx
  on repo_asm_memory_immediate_parametric_encoding_fact(repo_file_id, function_name, line_no);

create table repo_asm_base_memory_parametric_encoding_fact as
with register_memory as (
  select match.repo_file_id,
         match.function_name,
         match.line_no,
         match.op_name,
         memory.operand_index as memory_operand_index,
         memory.size_name,
         memory.integer_displacement as displacement,
         base_reg.reg_code as base_reg_code,
         base_reg.requires_rex as base_requires_rex,
  ...
  join repo_asm_binary_operand_fact reg
   ...
   and reg.function_name = match.function_name
   and reg.line_no = match.line_no
   and reg.operand_index = case when memory.operand_index = 0 then 1 else 0 end
   and reg.operand_kind = 'register'
  join x86_register_encoding_fact reg_encoding
    on reg_encoding.register_name = reg.operand_value
  where match.target_isa_id = 1
    ...
    and ((match.op_name = 'lea' and memory.operand_index = 1 and reg_encoding.width_bits in (32,64))
         ...
         or (match.op_name not in ('lea','movzx','movsx','movsxd')
             ...
             and (memory.size_name is null
                  ...
                  or (memory.size_name = 'word' and reg_encoding.width_bits = 16)
                  or (memory.size_name = 'dword' and reg_encoding.width_bits = 32)
                  or (memory.size_name = 'qword' and reg_encoding.width_bits = 64))))
), memory_bytes as (
  select register_memory.*,
         ...
         case when (base_reg_code & 7) = 4 then '24' else '' end as sib_hex,
         case when (base_reg_code & 7) = 4 then 4 else (base_reg_code & 7) end as rm_field
  from register_memory
), generated as (
  select repo_file_id,
         function_name,
         line_no,
         'param_x86_' || op_name || '_base_memory_reg_fact' as parametric_rule_name,
         (
           case when width_bits = 16 then '66' else '' end
           || case
                when width_bits = 64 then printf('%02x', 72 + case when reg_code >= 8 then 4 else 0 end + case when base_reg_code >= 8 then 1 else 0 end)
           ...
           || case
                when op_name = 'mov' and width_bits = 8 then '88'
                when op_name = 'mov' then '89'
                when op_name = 'add' and width_bits = 8 then '00'
                when op_name = 'add' then '01'
                when op_name = 'sub' and width_bits = 8 then '28'
                when op_name = 'sub' then '29'
                when op_name = 'cmp' and width_bits = 8 then '38'
                when op_name = 'cmp' then '39'
                when op_name = 'and' and width_bits = 8 then '20'
                when op_name = 'and' then '21'
                when op_name = 'or' and width_bits = 8 then '08'
                when op_name = 'or' then '09'
                when op_name = 'xor' and width_bits = 8 then '30'
                when op_name = 'xor' then '31'
                when op_name = 'test' and width_bits = 8 then '84'
                when op_name = 'test' then '85'
              end
           || printf('%02x', (mod_bits << 6) + ((reg_code & 7) << 3) + rm_field)
           || sib_hex
           || displacement_hex
  ...
  from memory_bytes
  where memory_operand_index = 0
    and op_name in ('mov','add','sub','cmp','and','or','xor','test')
  union all
  select repo_file_id,
         function_name,
         line_no,
         'param_x86_' || op_name || '_reg_base_memory_fact' as parametric_rule_name,
         (
           case when width_bits = 16 then '66' else '' end
           || case
                when width_bits = 64 then printf('%02x', 72 + case when reg_code >= 8 then 4 else 0 end + case when base_reg_code >= 8 then 1 else 0 end)
...
where fixed_hex is not null
  ...
  and not exists (select 1
                  ...
                  where previous.repo_file_id = generated.repo_file_id
                    ...
                    and previous.line_no = generated.line_no)
  and not exists (select 1
                  from repo_asm_memory_immediate_parametric_encoding_fact previous
                  where previous.repo_file_id = generated.repo_file_id
                    and previous.function_name = generated.function_name
                    and previous.line_no = generated.line_no);

create unique index repo_asm_base_memory_parametric_encoding_fact_line_idx
  on repo_asm_base_memory_parametric_encoding_fact(repo_file_id, function_name, line_no);
...
where fixed_hex is not null
  ...
  and not exists (select 1
                  ...
                  where previous.repo_file_id = generated.repo_file_id
                    ...
                    and previous.line_no = generated.line_no)
  and not exists (select 1
                  from repo_asm_base_memory_parametric_encoding_fact previous
                  where previous.repo_file_id = generated.repo_file_id
                    and previous.function_name = generated.function_name
                    and previous.line_no = generated.line_no);
...
create table repo_asm_sse_scalar_parametric_encoding_fact as
with binary_operand as (
  select match.repo_file_id,
         match.function_name,
         match.line_no,
  ...
  where match.target_isa_id = 1
    ...
    and match.encoding_id is null
    and match.rule_name is not null
    and match.op_name in ('movss','addss','subss','mulss','divss','ucomiss')
    and instr(coalesce(match.operand_text, ''), ',') > 0
), normalized as (
  select *,
         case when lhs like 'dword %' then trim(substr(lhs, 7)) else lhs end as lhs_value,
         case when rhs like 'dword %' then trim(substr(rhs, 7)) else rhs end as rhs_value
  from binary_operand
), memory_operand as (
  select repo_file_id,
         function_name,
  ...
  from (
    select repo_file_id,
           ...
           trim(replace(substr(memory_text, 2, length(memory_text) - 2), ' - ', ' + -')) as inner_text
    from (
      select repo_file_id, function_name, line_no, 'lhs' as side, lhs_value as memory_text
      from normalized
      union all
      select repo_file_id, function_name, line_no, 'rhs' as side, rhs_value as memory_text
      from normalized
    )
    where memory_text like '[%]'
  )
  ...
  join x86_register_encoding_fact base_reg
    on base_reg.register_name = memory_operand.base_register_name
   and base_reg.width_bits = 64
  where memory_operand.displacement is not null
), xmm_reg_reg as (
  select normalized.*,
         dst.reg_code as dst_reg_code,
         dst.requires_rex as dst_requires_rex,
         ...
         src.requires_rex as src_requires_rex
  from normalized
  join x86_xmm_register_encoding_fact dst on dst.register_name = normalized.lhs_value
  join x86_xmm_register_encoding_fact src on src.register_name = normalized.rhs_value
  where normalized.op_name in ('movss','addss','subss','mulss','divss','ucomiss')
), xmm_to_memory as (
  select normalized.*,
         src.reg_code as src_reg_code,
         src.requires_rex as src_requires_rex,
  ...
  from normalized
  join x86_xmm_register_encoding_fact src on src.register_name = normalized.rhs_value
  join memory_bytes mem
    on mem.repo_file_id = normalized.repo_file_id
   and mem.function_name = normalized.function_name
   and mem.line_no = normalized.line_no
   and mem.side = 'lhs'
  where normalized.op_name = 'movss'
), memory_to_xmm as (
  select normalized.*,
         dst.reg_code as dst_reg_code,
         dst.requires_rex as dst_requires_rex,
  ...
  select repo_file_id,
         function_name,
         line_no,
         'param_x86_' || op_name || '_xmm_xmm' as parametric_rule_name,
         (
           case
             when dst_requires_rex = 1 or src_requires_rex = 1 then printf('%02x', 64 + case when dst_reg_code >= 8 then 4 else 0 end + case when src_reg_code >= 8 then 1 else 0 end)
             else ''
           end
           || case when op_name = 'ucomiss' then '' else 'f3' end
           || case op_name
                when 'movss' then '0f10'
                when 'addss' then '0f58'
                when 'subss' then '0f5c'
                when 'mulss' then '0f59'
                when 'divss' then '0f5e'
                when 'ucomiss' then '0f2e'
              end
           || printf('%02x', 192 + ((dst_reg_code & 7) << 3) + (src_reg_code & 7))
         ) as fixed_hex
  from xmm_reg_reg
  ...
  select repo_file_id,
         function_name,
         line_no,
         'param_x86_movss_memory_xmm' as parametric_rule_name,
         (
           case
             when src_requires_rex = 1 or base_requires_rex = 1 then printf('%02x', 64 + case when src_reg_code >= 8 then 4 else 0 end + case when base_reg_code >= 8 then 1 else 0 end)
             else ''
           end
           || 'f30f11'
           || printf('%02x', (mod_bits << 6) + ((src_reg_code & 7) << 3) + rm_field)
           || sib_hex
           || displacement_hex
         ) as fixed_hex
  from xmm_to_memory
  ...
  select repo_file_id,
         function_name,
         line_no,
         'param_x86_' || op_name || '_xmm_memory' as parametric_rule_name,
         (
           case
             when dst_requires_rex = 1 or base_requires_rex = 1 then printf('%02x', 64 + case when dst_reg_code >= 8 then 4 else 0 end + case when base_reg_code >= 8 then 1 else 0 end)
             else ''
           ...
           || printf('%02x', (mod_bits << 6) + ((dst_reg_code & 7) << 3) + rm_field)
           || sib_hex
           || displacement_hex
         ) as fixed_hex
  from memory_to_xmm
...
where fixed_hex is not null
  ...
  and not exists (select 1
                  ...
                  where previous.repo_file_id = generated.repo_file_id
                    ...
                    and previous.line_no = generated.line_no)
  and not exists (select 1
                  from repo_asm_stack_alias_parametric_encoding_fact previous
                  where previous.repo_file_id = generated.repo_file_id
                    and previous.function_name = generated.function_name
                    and previous.line_no = generated.line_no);

create unique index repo_asm_sse_scalar_parametric_encoding_fact_line_idx
  on repo_asm_sse_scalar_parametric_encoding_fact(repo_file_id, function_name, line_no);

create table arm32_register_encoding_fact (
  register_name text primary key,
  reg_code integer not null
);

insert into arm32_register_encoding_fact(register_name, reg_code) values
  ('r0', 0), ('r1', 1), ('r2', 2), ('r3', 3),
  ('r4', 4), ('r5', 5), ('r6', 6), ('r7', 7),
...
create table arm32_condition_fact (
  op_name text primary key,
  base_op_name text not null,
  cond_code integer not null
);

insert into arm32_condition_fact(op_name, base_op_name, cond_code) values
...
with recursive operation as (
  select match.repo_file_id,
         ...
         match.op_name,
         trim(case
           when instr(match.operand_text, '@') > 0 then substr(match.operand_text, 1, instr(match.operand_text, '@') - 1)
           else match.operand_text
         end) as operand_text
  from repo_asm_rule_match match
  where match.target_isa_id = 4
    and match.line_kind = 3
    and match.encoding_id is null
    and match.rule_name is not null
  ...
  select operation.*,
         ...
         trim(substr(operand_text, instr(operand_text, ',') + 1)) as tail0
  from operation
  where instr(operand_text, ',') > 0
), split_three as (
  select split_operand.*,
         trim(substr(tail0, 1, instr(tail0, ',') - 1)) as arg1,
  ...
  select split_two.repo_file_id,
         ...
         rm.reg_code as rm_code
  from split_two
  join arm32_condition_fact cond on cond.op_name = split_two.op_name
  ...
  from split_two
  join arm32_condition_fact cond on cond.op_name = split_two.op_name
  join arm32_register_encoding_fact rn on rn.register_name = split_two.arg0
  join arm32_register_encoding_fact rm on rm.register_name = split_two.arg1
  where split_two.op_name in ('cmp','tst')
), arm_shift_imm as (
  select decimal_three_immediate.repo_file_id,
         decimal_three_immediate.function_name,
         decimal_three_immediate.line_no,
  ...
  select split_operand.repo_file_id,
         ...
         split_operand.line_no,
         split_operand.op_name,
  ...
  select memory_operand.*,
         ...
         trim(substr(memory_text, instr(memory_text, ']') + 1)) as post_text
  from memory_operand
  where memory_text like '[%]%'
    and instr(memory_text, ']') > 0
), memory_base_offset as (
  select memory_inner.*,
         trim(substr(inner_text, 1, instr(inner_text, ',') - 1)) as base_reg,
         trim(substr(inner_text, instr(inner_text, ',') + 1)) as offset_text
  from memory_inner
  where instr(inner_text, ',') > 0
    and trim(substr(inner_text, instr(inner_text, ',') + 1)) like '#%'
    and post_text = ''
), memory_base_only as (
  select memory_inner.*,
         inner_text as base_reg,
         '0' as offset_text
  from memory_inner
  where instr(inner_text, ',') = 0
    and post_text = ''
), memory_post_offset as (
  select memory_inner.*,
         inner_text as base_reg,
         trim(substr(post_text, 2)) as offset_text
  from memory_inner
  where instr(inner_text, ',') = 0
    and post_text like ', #%'
  ...
  select memory_base_only.repo_file_id,
         ...
         memory_base_only.line_no,
         memory_base_only.op_name,
         rd.reg_code as rd_code,
         rn.reg_code as rn_code,
         0 as offset_value,
         1 as pre_index
  from memory_base_only
  join arm32_register_encoding_fact rd on rd.register_name = memory_base_only.data_reg
  join arm32_register_encoding_fact rn on rn.register_name = memory_base_only.base_reg
  union all
  select memory_post_offset.repo_file_id,
         memory_post_offset.function_name,
         memory_post_offset.line_no,
  ...
  select repo_file_id,
         ...
         line_no,
         'param_arm32_' || op_name || '_reg' as parametric_rule_name,
         (cond_code << 28) + ((case base_op_name when 'add' then 4 when 'sub' then 2 when 'and' then 0 when 'orr' then 12 when 'bic' then 14 end) << 21)
           + (rn_code << 16) + (rd_code << 12) + rm_code as fixed_word
  from arm_dp_reg
  union all
  select repo_file_id,
         function_name,
         ...
         'param_arm32_' || op_name || '_reg_shift' as parametric_rule_name,
         (cond_code << 28) + ((case base_op_name when 'add' then 4 when 'sub' then 2 when 'and' then 0 when 'orr' then 12 when 'bic' then 14 end) << 21)
           + (rn_code << 16) + (rd_code << 12) + (shift_value << 7) + ((case shift_name when 'lsl' then 0 when 'lsr' then 1 end) << 5) + rm_code as fixed_word
  from arm_dp_shift_reg
  where shift_value between 0 and 31
  union all
  select repo_file_id,
         function_name,
         line_no,
         'param_arm32_' || op_name || '_imm_shift' as parametric_rule_name,
         (cond_code << 28) + (13 << 21) + (rd_code << 12) + (immediate_value << 7) + ((case op_name when 'lsl' then 0 when 'lsr' then 1 end) << 5) + rm_code as fixed_word
  from arm_shift_imm
  where immediate_value between 0 and 31
  union all
  select repo_file_id,
         function_name,
         line_no,
         'param_arm32_' || op_name || '_memory_imm12' as parametric_rule_name,
         (14 << 28) + (1 << 26) + (pre_index << 24) + (1 << 23)
  ...
  from arm_svc
  where immediate_value between 0 and 16777215
)
select repo_file_id,
       function_name,
       line_no,
       ...
       printf('%02x%02x%02x%02x',
              ...
              (fixed_word >> 8) & 255,
              (fixed_word >> 16) & 255,
              (fixed_word >> 24) & 255) as fixed_hex
from generated;

create unique index repo_asm_arm32_parametric_encoding_fact_line_idx
  on repo_asm_arm32_parametric_encoding_fact(repo_file_id, function_name, line_no);
...
union all
select *
from repo_asm_memory_immediate_parametric_encoding_fact
union all
select *
from repo_asm_base_memory_parametric_encoding_fact
union all
select *
...
union all
select *
from repo_asm_sse_scalar_parametric_encoding_fact
union all
select *
from repo_asm_arm32_parametric_encoding_fact;
