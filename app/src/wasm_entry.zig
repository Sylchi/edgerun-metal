...
pub const max_total_js_bytes: usize = 8550;
...
++ keyboard_capture_event_js ++
    ...
    \\let E={cell_available:s=>0,cell_recv:(s,p)=>81,cell_send:(h,c)=>82,route_unregister:s=>0,route_lookup:h=>82,route_register:h=>84};
    \\W=(await WebAssembly.instantiate(await(await fetch`../bin/edgerun-app-runtime.wasm`).arrayBuffer(),{env:L,er:E})).instance.exports;M=W.memory.buffer;W.er_ui_wasm_gl_init();W.er_ui_boot();P();A({type:`hashchange`},8);
...
test "generated entry has the only javascript byte bridge" {
    ...
    try std.testing.expect(contains("{env:L,er:E}"));
    try std.testing.expect(contains("cell_available"));
    try std.testing.expect(contains("route_register"));
    ...
    try std.testing.expect(contains("I=[I,WebAssembly.instantiate(U(p,l),{env:L,er:E})"));
