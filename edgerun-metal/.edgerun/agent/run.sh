...
NETBOOT_UNIT="${REPO_DIR}/systemd/edgerun-netboot.service"
...
deploy_netboot_unit() {
  if [ ! -f "${NETBOOT_UNIT}" ]; then
    log "[agent] netboot unit missing: ${NETBOOT_UNIT}"
    return 0
  fi
  if ! command -v sudo >/dev/null 2>&1; then
    log "[agent] sudo missing; cannot deploy netboot unit"
    return 1
  fi
  if ! sudo -n install -m 0644 "${NETBOOT_UNIT}" /etc/systemd/system/edgerun-netboot.service >>"${RUN_LOG}" 2>&1; then
    log "[agent] install netboot unit failed"
    return 1
  fi
  if ! sudo -n systemctl daemon-reload >>"${RUN_LOG}" 2>&1; then
    log "[agent] daemon-reload failed"
    return 1
  fi
  log "[agent] deployed netboot service unit"
}
