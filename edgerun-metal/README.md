...
Status: host-tested ingress records and first render endpoint-intent decode implemented; signed admission verification, OS loop integration, and device adapters are next.

`er_native_boot_poll_relay_ingress` accepts native erwire packets and records accepted, malformed, or empty ingress deterministically while preserving accepted payload bytes. Accepted relay packets can now be decoded against an admitted render capability route into a native endpoint intent. The durable path must finish signed admission verification, then hand assigned local endpoint adapters only already-admitted work.
