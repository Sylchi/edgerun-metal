...
static const char ERB_NODE_BUILD_DIR[] = ".build/er-build-out/node";
...
static const char ERB_NODE_TEST_BIN[] = ".build/er-build-out/node/test_node";
...
static int erb_target_node_test(int print_plan);
...
static int erb_target_repo_test(int print_plan) {
  ...
  if (erb_run_program("./tests/repo-check-tests.sh", print_plan) != 0 ||
      ...
      erb_target_node_test(print_plan) != 0) {
    return 1;
  }
  return 0;
}
...
static int erb_target_storage_test(int print_plan) {
  ...
  if (erb_args_push(&args, "-Iedgerun-storage/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-object/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-identity/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-clock/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-crypto/include") != 0 ||
      erb_args_push(&args, "-Iinclude") != 0 ||
      erb_args_push(&args, "edgerun-storage/tests/test_store.c") != 0 ||
      erb_args_push(&args, "edgerun-storage/src/er_store.c") != 0 ||
      erb_args_push(&args, "edgerun-object/src/er_object.c") != 0 ||
      erb_args_push(&args, "edgerun-identity/src/er_identity.c") != 0 ||
      erb_args_push(&args, "edgerun-clock/src/er_clock.c") != 0 ||
      erb_args_push(&args, "edgerun-crypto/src/er_blake3.c") != 0) {
...
static int erb_target_storage_bench(int print_plan) {
  ...
  if (erb_args_push(&args, "-Iedgerun-storage/include") != 0 ||
      ...
      erb_args_push(&args, "-Iedgerun-crypto/include") != 0 ||
      erb_args_push(&args, "-Iinclude") != 0 ||
      erb_args_push(&args, "edgerun-storage/bench/bench_store.c") != 0 ||
      erb_args_push(&args, "edgerun-storage/src/er_store.c") != 0 ||
      erb_args_push(&args, "edgerun-object/src/er_object.c") != 0 ||
      erb_args_push(&args, "edgerun-identity/src/er_identity.c") != 0 ||
      erb_args_push(&args, "edgerun-clock/src/er_clock.c") != 0 ||
...
static int erb_target_node_test(int print_plan) {
  ...
  if (erb_prepare_dirs() != 0 ||
      erb_mkdir_one(ERB_NODE_BUILD_DIR) != 0 ||
      erb_compile_common(&args, ERB_NODE_TEST_BIN) != 0) {
    return 1;
  }
  if (erb_args_push(&args, "-Iedgerun-node/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-storage/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-object/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-identity/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-clock/include") != 0 ||
      erb_args_push(&args, "-Iedgerun-crypto/include") != 0 ||
      erb_args_push(&args, "-Iinclude") != 0 ||
      erb_args_push(&args, "edgerun-node/tests/test_node.c") != 0 ||
      erb_args_push(&args, "edgerun-node/src/er_node.c") != 0 ||
      erb_args_push(&args, "edgerun-storage/src/er_store.c") != 0 ||
      erb_args_push(&args, "edgerun-object/src/er_object.c") != 0 ||
      erb_args_push(&args, "edgerun-identity/src/er_identity.c") != 0 ||
      erb_args_push(&args, "edgerun-clock/src/er_clock.c") != 0 ||
  ...
  return erb_run_program(ERB_NODE_TEST_BIN, print_plan);
}
...
static int erb_usage(void) {
  fprintf(stderr,
          ...
          "         crypto-test crypto-bench clock-test identity-test object-test storage-test storage-bench node-test varfont-test ui-core-test\n");
  return 2;
}
...
int main(int argc, char** argv) {
  ...
  if (strcmp(target, "node-test") == 0) {
    return erb_target_node_test(print_plan);
  }
