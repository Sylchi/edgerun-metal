...
static void eri_scan_line_metrics(const uint8_t* bytes, size_t len, EriTotals* file_totals,
  ...
  while (pos <= len) {
    ...
    if (end > start) {
      ...
      if (eri_line_has_magic_number(searchable, literal, sizeof(literal)) != 0u) {
        char text[144];
  ...
  while (pos <= file->len) {
    ...
    if (eri_normalize_code_line(file->bytes, start, end, normalized, sizeof(normalized)) != 0u) {
      if (len + 1u > cap) {
        ...
        if (grown_hashes == NULL || grown_lines == NULL) {
          free(grown_hashes);
          free(grown_lines);
          free(hashes);
          free(lines);
      ...
      hashes[len] = eri_hash_bytes(normalized, strlen(normalized));
      lines[len] = line_no;
      ++len;
    }
    ++line_no;
