...
static void er_pi_node_update_usage(const char* argv0) {
  fprintf(stderr,
          "usage: %s (--serial <tty> | --iface <edgenet-iface> | --dry-run) --image <kernel.img> [--repeat N]\n",
...
static int er_pi_node_update_open_l2(const char* iface,
  ...
  out_addr->sll_protocol = htons(ER_NET_ETH_TYPE_EDGERUN);
...
static int er_pi_node_update_send_packet(int fd,
  ...
  uint8_t frame[ER_PI_ZERO_W_V1_1_OTA_L2_FRAME_BYTES_MAX];
  uint32_t frame_len;
  ssize_t sent;
